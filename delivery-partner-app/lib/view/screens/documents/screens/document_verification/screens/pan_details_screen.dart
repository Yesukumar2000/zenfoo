...
class _PanDetailsScreenState extends State<PanDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _panNumberController.text = docProvider.panNumber ?? '';

      // Check if there's existing data from server
      _hasExistingData = docProvider.panFrontUrl != null ||
          docProvider.panBackUrl != null ||
          (docProvider.panNumber != null && docProvider.panNumber!.isNotEmpty);
    });
  }
  ...
  /// Show image picker for PAN card (front or back)
  void _showImagePicker(bool isFront) {
    final title = isFront ? 'Capture PAN Front' : 'Capture PAN Back';
    ...
    ImagePickerBottomSheet.show(
      ...
      onImageSelected: (file) {
        if (mounted) {
          final docProvider = context.read<DocumentProvider>();
          if (isFront) {
            docProvider.setPanFront(file);
          } else {
            docProvider.setPanBack(file);
          }
        }
      },
      onPermissionDenied: () {
  ...
  void _removeImage(bool isFront) {
    final docProvider = context.read<DocumentProvider>();
    if (isFront) {
      docProvider.setPanFront(null);
    } else {
      docProvider.setPanBack(null);
    }
  }
  ...
  Future<void> _submit() async {
    ...
    final hasNewFrontImage = docProvider.panFront != null;
    final hasNewBackImage = docProvider.panBack != null;
    final hasNewNumber = _panNumberController.text.trim().isNotEmpty;
    ...
    if (!_hasExistingData) {
      if (!hasNewFrontImage || !hasNewBackImage || !hasNewNumber) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please upload both images and enter PAN number'),
          ),
        );
        return;
      }
    ...
    try {
      // Update the PAN number in provider
      if (hasNewNumber) {
        docProvider.setPanNumber(_panNumberController.text.trim());
      }

      // Call updateDocuments API
      final response = await _documentRepo.updateDocuments(
        panNumber: hasNewNumber ? _panNumberController.text.trim() : null,
        panFront: docProvider.panFront,
        panBack: docProvider.panBack,
      );
      ...
      if (mounted) {
        ...
        if (isStatusSuccess(response.status)) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('PAN details uploaded successfully'),
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      CustomTextFormField(
                        ...
                        onChanged: (value) {
                          // Update in real-time
                          docProvider.setPanNumber(value);
                        },
                      ),
                      ...
                      _buildUploadBox(
                        ...
                        image: docProvider.panFront,
                        imageUrl: docProvider.panFrontUrl,
                      ...
                      _buildUploadBox(
                        ...
                        textTheme: textTheme,
                        title: "Back Side",
                        subtitle: "Upload back side of PAN card",
                        image: docProvider.panBack,
                        imageUrl: docProvider.panBackUrl,
