...
class RcDetailsScreen extends StatefulWidget {
  ...
  @override
  State<RcDetailsScreen> createState() => _RcDetailsScreenState();
}
...
class _RcDetailsScreenState extends State<RcDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _rcNumberController.text = docProvider.rcNumber ?? '';

      // Check if there's existing data from server
      _hasExistingData = docProvider.rcFrontUrl != null ||
          docProvider.rcBackUrl != null ||
          (docProvider.rcNumber != null && docProvider.rcNumber!.isNotEmpty);
    });
  }
  ...
  /// Show image picker for RC (front or back)
  void _showImagePicker(bool isFront) {
    final title = isFront ? 'Capture RC Front' : 'Capture RC Back';
    ...
    ImagePickerBottomSheet.show(
      ...
      onImageSelected: (file) {
        if (mounted) {
          final docProvider = context.read<DocumentProvider>();
          if (isFront) {
            docProvider.setRcFront(file);
          } else {
            docProvider.setRcBack(file);
          }
        }
      },
      onPermissionDenied: () {
  ...
  void _removeImage(bool isFront) {
    final docProvider = context.read<DocumentProvider>();
    if (isFront) {
      docProvider.setRcFront(null);
    } else {
      docProvider.setRcBack(null);
    }
  }
  ...
  Future<void> _submit() async {
    ...
    final hasNewFrontImage = docProvider.rcFront != null;
    final hasNewBackImage = docProvider.rcBack != null;
    final hasNewNumber = _rcNumberController.text.trim().isNotEmpty;
    ...
    if (!_hasExistingData) {
      if (!hasNewFrontImage || !hasNewBackImage || !hasNewNumber) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please upload both images and enter RC number'),
          ),
        );
        return;
      }
    ...
    try {
      // Update the RC number in provider
      if (hasNewNumber) {
        docProvider.setRcNumber(_rcNumberController.text.trim());
      }

      // Call updateDocuments API
      final response = await _documentRepo.updateDocuments(
        rcNumber: hasNewNumber ? _rcNumberController.text.trim() : null,
        rcFront: docProvider.rcFront,
        rcBack: docProvider.rcBack,
      );
      ...
      if (mounted) {
        ...
        if (isStatusSuccess(response.status)) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('RC details uploaded successfully'),
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      _buildUploadBox(
                        ...
                        textTheme: textTheme,
                        title: "Front Side",
                        subtitle: "Upload front side of RC",
                        image: docProvider.rcFront,
                        imageUrl: docProvider.rcFrontUrl,
                      ...
                      _buildUploadBox(
                        ...
                        textTheme: textTheme,
                        title: "Back Side",
                        subtitle: "Upload back side of RC",
                        image: docProvider.rcBack,
                        imageUrl: docProvider.rcBackUrl,
