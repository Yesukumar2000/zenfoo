...
class _DrivingLicenseScreenState extends State<DrivingLicenseScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _licenseNumberController.text = docProvider.drivingLicenseNumber ?? '';

      // Check if there's existing data from server
      _hasExistingData = docProvider.drivingLicenseFrontUrl != null ||
          docProvider.drivingLicenseBackUrl != null ||
  ...
  /// Show image picker for driving license (front or back)
  void _showImagePicker(bool isFront) {
    final title = isFront ? 'Capture License Front' : 'Capture License Back';
    ...
    ImagePickerBottomSheet.show(
      ...
      onImageSelected: (file) {
        if (mounted) {
          final docProvider = context.read<DocumentProvider>();
          if (isFront) {
            docProvider.setDrivingLicenseFront(file);
          } else {
            docProvider.setDrivingLicenseBack(file);
          }
        }
      },
      onPermissionDenied: () {
  ...
  void _removeImage(bool isFront) {
    final docProvider = context.read<DocumentProvider>();
    if (isFront) {
      docProvider.setDrivingLicenseFront(null);
    } else {
      docProvider.setDrivingLicenseBack(null);
    }
  }
  ...
  Future<void> _submit() async {
    ...
    final hasNewFrontImage = docProvider.drivingLicenseFront != null;
    final hasNewBackImage = docProvider.drivingLicenseBack != null;
    final hasNewNumber = _licenseNumberController.text.trim().isNotEmpty;
    ...
    if (!_hasExistingData) {
      if (!hasNewFrontImage || !hasNewBackImage || !hasNewNumber) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please upload both images and enter license number'),
          ),
        );
        return;
      }
    ...
    try {
      // Update the license number in provider
      if (hasNewNumber) {
        docProvider
            .setDrivingLicenseNumber(_licenseNumberController.text.trim());
      }

      // Call updateDocuments API
      final response = await _documentRepo.updateDocuments(
        drivingLicenseNumber:
            hasNewNumber ? _licenseNumberController.text.trim() : null,
        drivingLicenseFront: docProvider.drivingLicenseFront,
        drivingLicenseBack: docProvider.drivingLicenseBack,
      );
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      CustomTextFormField(
                        ...
                        onChanged: (value) {
                          // Update in real-time
                          docProvider.setDrivingLicenseNumber(value);
                        },
                      ),
                      ...
                      _buildUploadBox(
                        ...
                        textTheme: textTheme,
                        title: "Front Side",
                        subtitle: "Upload front side of driving license",
                        image: docProvider.drivingLicenseFront,
                        imageUrl: docProvider.drivingLicenseFrontUrl,
                      ...
                      _buildUploadBox(
                        ...
                        textTheme: textTheme,
                        title: "Back Side",
                        subtitle: "Upload back side of driving license",
                        image: docProvider.drivingLicenseBack,
                        imageUrl: docProvider.drivingLicenseBackUrl,
