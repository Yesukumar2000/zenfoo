...
import 'package:zenfoo_partner/models/multi_order_issue_model.dart';
...
class MultiOrderIssuesScreen extends StatefulWidget {
  const MultiOrderIssuesScreen({super.key});

  @override
  State<MultiOrderIssuesScreen> createState() => _MultiOrderIssuesScreenState();
}

class _MultiOrderIssuesScreenState extends State<MultiOrderIssuesScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PayoutIssuesProvider>().multiOrderWeeks();
    });
  }
        ...
        children: [
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                      ...
                      Container(
                        ...
                        child: Consumer<PayoutIssuesProvider>(
                          builder: (context, provider, child) {
                            if (provider.multiOrderWeeksState.status ==
                                ApiStatus.loading) {
                              return const Center(
                                child: SizedBox(
                                  width: 20,
                            ...
                            final weekModel =
                                provider.multiOrderWeeksState.data != null
                                    ? WeekModel.fromJson(
                                        provider.multiOrderWeeksState.data)
                                    : null;
                  ...
                  // Multi-Order Summary and Transactions
                  if (_selectedWeek != null)
                    Consumer<PayoutIssuesProvider>(
                      builder: (context, provider, child) {
                        if (provider
                                .getWeekMultiOrderTransactionsState.status ==
                            ApiStatus.loading) {
                          return const Center(
                            child: Padding(
                        ...
                        final transactionModel =
                            provider.getWeekMultiOrderTransactionsState.data !=
                                    null
                                ? MultiOrderIssueModel.fromJson(provider
                                    .getWeekMultiOrderTransactionsState.data)
                                : null;
                                    ...
                                    return CheckboxListTile(
                                      ...
                                      title: Text(
                                        transaction.description != null
                                            ? "${transaction.description} #${transaction.orderId ?? ''}"
                                      ...
                                      subtitle: Text(
                                        '₹${transaction.amount ?? '0'} • ${transaction.createdAt?.toString().split(' ')[0] ?? ''}',
                                        style: GoogleFonts.inter(
                                          color: colorScheme.textSecondary,
                                          fontSize: 12,
                                        ),
          ...
          Padding(
            ...
            child: Consumer<PayoutIssuesProvider>(
              builder: (context, provider, child) {
                final isLoading =
                    provider.multiOrderBankState.status == ApiStatus.loading;
                return CustomButton(
  ...
  Widget _buildMultiOrderSummary(
    Summary summary,
    dynamic colorScheme,
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                languageProvider.getTranslatedText('multi_order_summary'),
                style: GoogleFonts.inter(
                  color: colorScheme.textPrimary,
                  fontSize: 16,
  ...
  Future<void> _handleSubmit() async {
    ...
    await provider.multiOrderBank(
      description: _descriptionController.text.trim(),
      issueType: 'multi_order',
      issueIds: _selectedTransactions.toList(),
      attachments: _selectedImage != null ? [_selectedImage!] : null,
    );

    if (mounted) {
      if (provider.multiOrderBankState.status == ApiStatus.success) {
        Navigator.push(
          context,
          MaterialPageRoute(
