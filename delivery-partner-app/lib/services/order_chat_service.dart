  ...
  factory ChatMessage.fromJson(Map<String, dynamic> json) {
    return ChatMessage(
      ...
      timestamp: json['timestamp'] is Timestamp
          ? toIst((json['timestamp'] as Timestamp).toDate())
          : toIst(DateTime.parse(
...
class OrderChatService {
  ...
  /// Get chat path based on sender and receiver type
  ...
  /// Get real-time stream of all messages in a chat
  Stream<List<ChatMessage>> getChatMessagesStream(int orderId, {String senderType = 'driver', String receiverType = 'seller'}) {
    try {
  ...
  /// Get messages sent by a specific sender type
  ...
  /// Add message to Firestore (called after API confirms)
  Future<void> addMessage({
    ...
    try {
      final chatPath = _getChatPath(senderType, receiverType);
      final messageId = 'msg_${DateTime.now().millisecondsSinceEpoch.toString()}';
      ...
      debugPrint('✅ Message added to Firestore: /chatting/$orderId/$chatPath/$messageId');
    } catch (e) {
  ...
  Future<void> markMessageAsRead(int orderId, String messageId, {String senderType = 'driver', String receiverType = 'seller'}) async {
    try {
      final chatPath = _getChatPath(senderType, receiverType);
      await _firestore
          .collection('chatting')
  ...
  /// Mark all unread messages from a receiver type as read
  Future<void> markAllMessagesAsRead(
    ...
    try {
      ...
      debugPrint(
          '✅ Marked ${unreadMessages.docs.length} messages as read from $senderType');
    } catch (e) {
  ...
  Future<void> deleteMessage(int orderId, String messageId, {String senderType = 'driver', String receiverType = 'seller'}) async {
    try {
      final chatPath = _getChatPath(senderType, receiverType);
      await _firestore
          .collection('chatting')
  ...
  Future<ChatMessage?> getLastMessage(int orderId, {String senderType = 'driver', String receiverType = 'seller'}) async {
    try {
      final chatPath = _getChatPath(senderType, receiverType);
      final snapshot = await _firestore
          .collection('chatting')
  ...
  /// Create or get chat metadata for an order
  Future<Map<String, dynamic>> getChatMetadata(int orderId, {String senderType = 'driver', String receiverType = 'seller'}) async {
    try {
      final chatPath = _getChatPath(senderType, receiverType);
      ...
      debugPrint('⚠️ Error getting chat metadata: $e');
      return {};
    }
  }
  ...
  Future<void> updateChatMetadata(
    ...
    try {
      ...
      debugPrint('⚠️ Error updating chat metadata: $e');
    }
  }
}
