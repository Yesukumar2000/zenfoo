    ...
    <div class="chat-wrapper">
        ...
        <div class="chat-header d-flex align-items-center justify-content-between p-3">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h6 class="mb-0">{{ deliveryBoyName || 'Driver' }}</h6>
                    <small class="text-muted">ID: {{ deliveryBoyId }}</small>
                </div>
            </div>
            <div class="d-flex align-items-center">
        ...
        <div class="chat-messages" ref="chatMessages">
            ...
            <div v-else class="messages-wrapper p-3">
                <div v-for="(msg, index) in messages" :key="index" class="message-row" :class="{ 'message-sent': msg.sender === 'admin', 'message-received': msg.sender === 'delivery_boy' }">
                    <div class="message-bubble" :class="{ 'sent': msg.sender === 'admin', 'received': msg.sender === 'delivery_boy' }">
...
<script>
import { subscribeToDeliveryBoyChatMessages } from '../../services/FirebaseService';
    ...
    methods: {
        async initializeChat() {
            ...
            try {
                ...
                await axios.post(this.$apiUrl + '/delivery-boy-chat/initialize', {
                    delivery_boy_id: this.deliveryBoyId
                });

                // Fetch delivery boy details
                await this.fetchDeliveryBoyDetails();
        ...
        async fetchDeliveryBoyDetails() {
            try {
                const response = await axios.get(this.$apiUrl + '/delivery-boy-chat/' + this.deliveryBoyId + '/messages?limit=1');
                if (response.data.status === 1 && response.data.data.delivery_boy) {
                    this.deliveryBoy = response.data.data.delivery_boy;
                    this.deliveryBoyName = this.deliveryBoy.name || '';
                }
            } catch (error) {
        ...
        async startListening() {
            try {
                ...
                this.unsubscribe = await subscribeToDeliveryBoyChatMessages(
                    this.deliveryBoyId,
                    (messages, error) => {
                        if (error) {
        ...
        async loadMessagesViaApi() {
            try {
                const response = await axios.get(this.$apiUrl + '/delivery-boy-chat/' + this.deliveryBoyId + '/messages');
                if (response.data.status === 1) {
                    this.messages = response.data.data.messages || [];
                    if (response.data.data.delivery_boy) {
                        this.deliveryBoy = response.data.data.delivery_boy;
                        this.deliveryBoyName = this.deliveryBoy.name || '';
                    }
                    this.scrollToBottom(false);
        ...
        async markAsRead() {
            try {
                await axios.post(this.$apiUrl + '/delivery-boy-chat/mark-read', {
                    delivery_boy_id: this.deliveryBoyId
                });
            } catch (error) {
        ...
        async sendMessage() {
            ...
            try {
                const response = await axios.post(this.$apiUrl + '/delivery-boy-chat/send', {
                    delivery_boy_id: this.deliveryBoyId,
                    message: messageText
                });
