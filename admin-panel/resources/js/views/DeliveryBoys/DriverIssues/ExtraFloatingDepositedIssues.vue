    ...
    <div>
        ...
        <b-table
            ...
            <template #cell(amount)="row">
                <span v-if="row.item.amount" class="fw-bold text-success">
            ...
            <template #cell(pay_type)="row">
                <span v-if="row.item.pay_type" class="badge" :class="row.item.pay_type === 'upi' ? 'bg-info' : 'bg-primary'">
                    {{ row.item.pay_type.toUpperCase() }}
                </span>
                <span v-else class="text-muted">-</span>
            </template>
...
<script>
export default {
    name: 'ExtraFloatingDepositedIssues',
    props: {
        cityId: {
            type: [String, Number],
            default: ''
    ...
    data() {
        return {
            issueType: 'extra_floating_deposited',
            isLoading: false,
            issues: [],
            filter: '',
            filterStatus: '',
