<template>
  <b-modal ref="my-modal" :title="modal_title" @hidden="$emit('modalClose')" size="lg" scrollable no-close-on-backdrop no-fade static>
    ...
    <form ref="my-form" @submit.prevent="saveRecord">
      ...
      <div class="form-group col-md-6">
          <label>Is Promo Only For Some Sellers and Stores ? </label>
          <select v-model="is_selected_sellers" class="form-control">
      ...
      <div class="form-group col-md-6" v-if="Number(is_selected_sellers) === 1">
        <label>Select Sellers</label>

        <div v-for="seller in sellersList" :key="seller.id" class="custom-select-item"
            @click="toggleSeller(seller.id)">
          <input type="checkbox" :checked="seller_ids.includes(seller.id)" />
          {{ seller.store_name }}
        </div>
      </div>
      ...
      <div class="form-group col-md-6" v-if="Number(is_selected_sellers) === 1">
        ...
        <div v-for="store in storeList" :key="store.id" class="custom-select-item"
            @click="toggleStore(store.id)">
          <input type="checkbox" :checked="store_ids.includes(store.id)" />
          {{ store.name }}
        </div>
      </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      no_of_users: this.record && this.record.no_of_users ? this.record.no_of_users : 1000,
      ...
      is_selected_sellers: this.record ? Number(this.record.is_specific_sellers) : 0,

      // seller_ids: this.record ? this.record.seller_ids?.split(",") : [],
      sellersList: [],
      storeList: [],
      // store_ids: this.record ? this.record.store_ids?.split(",") : [],

      seller_ids: this.record && this.record.seller_ids
        ? this.record.seller_ids.split(",").map(id => Number(id))
        : [],

      store_ids: this.record && this.record.store_ids
        ? this.record.store_ids.split(",").map(id => Number(id))
        : [],
  ...
  methods: {
    toggleSeller(id) {
      id = Number(id);
      if (this.seller_ids.includes(id)) {
        this.seller_ids = this.seller_ids.filter(s => s !== id);
      } else {
        this.seller_ids.push(id);
      }
    },

    toggleStore(id) {
      id = Number(id);
      if (this.store_ids.includes(id)) {
        this.store_ids = this.store_ids.filter(s => s !== id);
      } else {
        this.store_ids.push(id);
      }
    },
    ...
    saveRecord: function () {
      ...
      formData.append("is_selected_sellers", this.is_selected_sellers);

      if(Number(this.is_selected_sellers) === 1){

        formData.append("seller_ids", this.seller_ids.join(","));
        formData.append("store_ids", this.store_ids.join(","));
      
      } else{
      
        formData.append("seller_ids", "");
        formData.append("store_ids", "");
  ...
  mounted() {
    ...
    axios.get(this.$apiUrl + "/promo_code/get-sellers-list")
    .then(res => {
    ...
    .then(res => {
      // console.log("get all stores = ", res);
