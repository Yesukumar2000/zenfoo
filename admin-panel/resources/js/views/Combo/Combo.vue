      ...
      <div class="card-body">
        ...
        >
          ...
          <router-link
            ...
            :to="{ name: 'EditCombo', params: { id: row.item.id } }"
            v-b-tooltip.hover
            :title="__('edit')"
          >
...
<script>
import AppEditCombo from "./EditCombo.vue";

export default {
  name: "CombosList",
  components: { AppEditCombo },
  data() {
    return {
      fields: [
  ...
  methods: {
    getCombos() {
        ...
        axios
          .get(this.$apiUrl + "/combos", { params })
          .then((res) => {
            this.isLoading = false;

            const combos = res.data;
            console.log("Freee Data :", combos);
    ...
    deleteCombo(index, id) {
      this.$swal
        ...
        .then((result) => {
          if (result.value) {
            axios.post(this.$apiUrl + "/combos/delete/combo", { id }).then(() => {
