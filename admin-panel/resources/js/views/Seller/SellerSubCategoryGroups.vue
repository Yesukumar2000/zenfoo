    ...
    <div>
        <b-row class="mb-2">
            <b-col md="3">
                <button class="btn btn-success" @click="openCreateModal()">
                    <i class="fa fa-plus me-1"></i> Add Sub Category Group
                </button>
            </b-col>
        ...
        <b-table
            ...
            <template #cell(actions)="row">
                ...
                <button class="btn btn-sm btn-danger" @click="deleteRecord(row.item.id)" v-b-tooltip.hover :title="__('delete')">
        ...
        <b-modal
            ...
            :title="isEditMode ? 'Edit Sub Category Group' : 'Add Sub Category Group'"
            hide-footer
            centered
            size="lg"
            scrollable>
            <form @submit.prevent="saveRecord">
                ...
                <div class="form-group mb-3">
                    <label>Select Categories <i class="text-danger">*</i></label>
                    <div class="border rounded p-3" :class="{ 'border-danger': showCategoryError }" style="max-height: 300px; overflow-y: auto;">
                        <div v-for="cat in sellerCategories" :key="cat.id" class="form-check mb-2">
                            <input
                                type="checkbox"
                                :id="'cat_' + cat.id"
                                :value="cat.id"
                                ...
                                @change="showCategoryError = false"
                            >
                            <label :for="'cat_' + cat.id" class="form-check-label">{{ cat.name }}</label>
                        </div>
                        <div v-if="sellerCategories.length === 0" class="text-muted">
                            No categories found for this seller
                        </div>
                    </div>
                    <small v-if="showCategoryError" class="text-danger">Please select at least one category</small>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            fields: [
                ...
                { key: 'subcategories', label: __('categories'), class: 'text-center' },
    ...
    methods: {
        ...
        fetchSellerCategories() {
            ...
            .then(res => {
                this.sellerCategories = res.data.data || [];
            })
            .catch(() => {
        ...
        getSubcategoryName(id) {
            let cat = this.sellerCategories.find(c => c.id == id);
        ...
        openEditModal(record) {
            ...
            this.editForm = {
                ...
                subcategory_ids: record.subcategory_ids ? record.subcategory_ids.split(',').map(Number) : [],
        ...
        saveRecord() {
            ...
            if (this.editForm.subcategory_ids.length === 0) {
            ...
            formData.append('category_ids', this.editForm.subcategory_ids.join(','));
            formData.append('seller_id', this.sellerId);
            if (this.editForm.image) {
                formData.append('image', this.editForm.image);
            }

            let url = this.$apiUrl + '/sellers/view/store-sub-category-group';
            if (this.isEditMode) {
                formData.append('id', this.editForm.id);
                formData.append('subcategory_ids', this.editForm.subcategory_ids.join(','));
                formData.append('category_group_id', this.editForm.category_group_id);
                formData.append('status', 1);
                formData.append('is_group', 0);
                url = this.$apiUrl + '/group-sub-category/update';
            }
