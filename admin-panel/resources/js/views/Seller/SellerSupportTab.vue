    ...
    <div class="chat-wrapper">
        ...
        <div class="chat-header d-flex align-items-center justify-content-between p-3">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h6 class="mb-0">{{ sellerName || 'Seller' }}</h6>
                    <small class="text-muted">ID: {{ sellerId }}</small>
                </div>
            </div>
            <div class="d-flex align-items-center">
        ...
        <div class="chat-messages" ref="chatMessages">
            ...
            <div v-else class="messages-wrapper p-3">
                <div v-for="(msg, index) in messages" :key="msg.id || index" class="message-row" :class="{ 'message-sent': msg.sender === 'admin', 'message-received': msg.sender === 'seller' }">
                    <div class="message-bubble" :class="{ 'sent': msg.sender === 'admin', 'received': msg.sender === 'seller' }">
...
<script>
import { subscribeToSellerChatMessages } from '../../services/FirebaseService';
    ...
    methods: {
        async initializeChat() {
            ...
            try {
                ...
                await axios.post(this.$apiUrl + '/seller-chat/initialize', {
        ...
        async fetchSellerDetails() {
            try {
                const response = await axios.get(this.$apiUrl + '/seller-chat/' + this.sellerId + '/messages?limit=1');
                if (response.data.status === 1 && response.data.data.seller) {
        ...
        async startListening() {
            try {
                ...
                this.unsubscribe = await subscribeToSellerChatMessages(
                    this.sellerId,
                    (messages, error) => {
                        if (error) {
        ...
        async loadMessagesViaApi() {
            try {
                const response = await axios.get(this.$apiUrl + '/seller-chat/' + this.sellerId + '/messages');
        ...
        async markAsRead() {
            try {
                await axios.post(this.$apiUrl + '/seller-chat/mark-read', {
                    seller_id: this.sellerId
                });
            } catch (error) {
        ...
        async sendMessage() {
            ...
            try {
                const response = await axios.post(this.$apiUrl + '/seller-chat/send', {
                    seller_id: this.sellerId,
                    message: messageText
                });
