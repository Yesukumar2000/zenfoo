        ...
        <section class="section">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div>
                        <h4 class="card-title mb-0">Pending Pre Orders</h4>
                        <p class="text-muted small mb-0">Orders awaiting processing (Status: Preorder Pending)</p>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-warning d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <span><strong>{{ totalOrderRows }}</strong> pending preorders awaiting processing</span>
                        </div>
                        <div>
...
export default {
    ...
    data() {
        return {
            ...
            stores: [],
            storeWiseData: [],
            status: '12', // Fixed to pending status
    ...
    methods: {
        ...
        async downloadPDF() {
            try {
                ...
                if (response.data.status === 1) {
                    ...
                    const printContent = this.generatePendingPrintContent(orders, date, totalAmount);
        ...
        generatePendingPrintContent(orders, date, totalAmount) {
            let rows = '';
            ...
            return `
                ...
                <html>
                <head>
                    <title>Pending Pre Orders - ${date}</title>
                    <style>
                        @page { size: A4 landscape; margin: 1cm; }
                        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
                        body { font-family: Arial, sans-serif; font-size: 10px; color: #333; padding: 20px; }
                        .header { text-align: center; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #f39c12; }
                        .header h1 { font-size: 24px; color: #2c3e50; margin-bottom: 5px; }
                        .header h2 { font-size: 18px; color: #f39c12; margin-bottom: 5px; }
                        .header p { font-size: 12px; color: #7f8c8d; }
                        table { width: 100%; border-collapse: collapse; margin-top: 10px; }
                        thead { background-color: #f39c12 !important; color: white !important; }
                        th { padding: 8px 5px; text-align: left; font-size: 9px; font-weight: bold; background-color: #f39c12 !important; color: white !important; }
                        td { padding: 6px 5px; border-bottom: 1px solid #ddd; font-size: 9px; }
                        tbody tr:nth-child(even) { background-color: #fff3cd !important; }
                        .text-right { text-align: right; }
                        .summary { margin-top: 20px; padding: 15px; background-color: #fff3cd !important; border: 2px solid #f39c12; border-radius: 5px; }
                ...
                <body>
                    <div class="header">
                        <h1>ZENFOO - PENDING PRE ORDERS</h1>
                    ...
                    <div class="summary">
                        <div class="summary-row"><span>Total Pending Orders:</span><strong>${orders.length}</strong></div>
                        <div class="summary-row"><span>Total Pending Amount:</span><strong>₹${parseFloat(totalAmount).toFixed(2)}</strong></div>
                    </div>
                    <div class="footer">
        ...
        async downloadExcel() {
            try {
                ...
                link.href = url;
                link.setAttribute('download', `pending-preorders-${moment().format('YYYY-MM-DD')}.xlsx`);
        ...
        async bulkPrintAll() {
            if (this.orders.length === 0) {
                this.$toast.warning('No pending orders to print');
                return;
            }

            this.$toast.info(`Preparing to print ${this.orders.length} pending orders...`);
            ...
            this.$toast.success('All pending orders sent to printer');
        }
    }
}
</script>
