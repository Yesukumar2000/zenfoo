                    ...
                    <div class="col-12 col-md-6 order-md-2 order-first">
                        <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"> {{ __('send_email') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <span class="pull-right">
                            <button class="btn btn-primary"  @click="create_new=true" v-if="$can('create_email')">{{ __('send_email') }}</button>
                        </span>
                    </div>
                    <div class="card-body">
                        ...
                        <b-row class="mb-2">
                            ...
                            <b-col md="1" class="text-center">
                                <button class="btn btn-primary btn_refresh" v-b-tooltip.hover :title="__('refresh')" @click="getEmails()">
                        ...
                        <div class="table-responsive">
                            <b-table
                                ...
                                <!-- <template #cell(message)="row">
                                    <div class="email_table_message" v-html="row.item.message"></div>
                                </template> -->
                                <template #cell(image)="row">
                                    <p v-if="row.item.image ===''">{{ __('no_image') }}</p>
                                    <img :src="$mediaUrl(row.item.image)" height="50" v-else/>
                                </template>
                                <template #cell(actions)="row">
                                    <button class="btn btn-sm btn-danger" @click="deleteEmail(row.index,row.item.id)"  v-if="$can('delete_email')" v-b-tooltip.hover :title="__('delete')"><i class="fa fa-trash"></i></button>
                                </template>
                                <template #cell(message)="row">
