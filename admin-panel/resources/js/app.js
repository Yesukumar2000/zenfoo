...
// Example usage
// console.log('window.MapApiKey:', window);
...
Vue.use(VueGoogleMaps, {
  load: {
    key: window.MapApiKey || decryptedMapKey || decryptedKey,
    libraries: 'drawing,places', // drawing + places
    // Add async and defer automatically
    loadCn: false,
  },
  installComponents: true, // Automatically installs <GmapMap>, <GmapMarker>, etc.
});
...
// Helper: returns the correct image URL whether it's a full S3 URL or a relative path
Vue.prototype.$mediaUrl = function(path) {
    if (!path) return '';
    return path.startsWith('http') ? path : window.baseUrl + '/storage/' + path;
};
