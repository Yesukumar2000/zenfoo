...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    getPosOrders: function getPosOrders() {
      ...
      axios.get(this.$sellerApiUrl + '/reports', {
        params: params
      }).then(function (response) {
        _this.isLoading = false;
        if (response.data.status) {
          _this.posOrders = response.data.data;
          _this.filteredItems = _toConsumableArray(_this.posOrders);

          // Debug: Log the structure of the data
          console.log("POS Orders Data:", _this.posOrders.length > 0 ? _this.posOrders[0] : 'No orders');
          console.log("Fields:", _this.fields);
          _this.totalRows = _this.filteredItems.length;
          _this.calculateFooterTotal();
        ...
        _this.isLoading = false;
        _this.showError("Error fetching POS orders");
      });
    },
    formatDate: function formatDate(dateString) {
      return moment__WEBPACK_IMPORTED_MODULE_1___default()(dateString).format('DD/MM/YYYY');
    },
    ...
    onFilterChanged: function onFilterChanged() {
      ...
      if (this.filter) {
        ...
        this.filteredItems = this.posOrders.filter(function (item) {
          return _this2.fields.some(function (field) {
