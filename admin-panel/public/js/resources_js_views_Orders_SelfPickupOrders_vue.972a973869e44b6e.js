...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  computed: {
    ...
    filteredOrderFields: function filteredOrderFields() {
      if (this.isSeller) {
        return this.orderFields.filter(function (field) {
  ...
  methods: {
    ...
    getSelfPickupOrderStatus: function getSelfPickupOrderStatus(tabTitle) {
      var _this = this;
      var vm = this;
      axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + '/order_statuses/self_pickup').then(function (response) {
    ...
    getSelfPickupOrders: function getSelfPickupOrders() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + apiEndpoint, {
        params: param
      }).then(function (response) {
        if (!_this2.isSeller && response.data.data.sellers) {
          _this2.sellers = response.data.data.sellers;
        }
        _this2.orders = response.data.data.orders;
        _this2.totalOrderRows = response.data.data.orders_total;
        _this2.total_amount = _this2.orders.map(function (item) {
          return Number(item.total);
        }).reduce(function (prev, curr) {
          return prev + curr;
        }, 0).toFixed(2);
        _this2.additional_charges_total = _this2.orders.map(function (item) {
          return _this2.getAdditionalChargesTotal(item.additional_charges || []);
        }).reduce(function (prev, curr) {
          return prev + curr;
        }, 0).toFixed(2);
    ...
    getAdditionalChargesTooltip: function getAdditionalChargesTooltip(charges) {
      ...
      return charges.map(function (charge) {
        return "".concat(charge.title, ": ").concat(_this4.$currency).concat(_this4.formatAmount(charge.amount));
      }).join('\n');
    },
                    ...
                    [
                      _c(
                        "button",
                        {
                          ...
                          on: {
                            click: function ($event) {
                              return _vm.getSelfPickupOrders()
                            },
                          },
                        },
                        [
                ...
                [
                  _c("b-table", {
                    attrs: {
                      responsive: "sm",
                      items: _vm.orders,
                      fields: _vm.filteredOrderFields,
                      filter: _vm.filter,
