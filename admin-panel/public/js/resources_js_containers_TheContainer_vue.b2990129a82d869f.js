...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  computed: {
    ...
    showBackButton: function showBackButton() {
      // Hide on top-level / dashboard / auth pages where "back" makes no sense
      var path = this.$route.path.replace(/\/$/, '') || '/';
      var rootPaths = ['/', '/dashboard', '/seller', '/seller/dashboard', '/delivery_boy', '/delivery_boy/dashboard', '/login', '/seller/login', '/delivery_boy/login', '/register', '/seller/register', '/delivery_boy/register', '/forgot-password', '/reset-password', '/install', '/unauthorized'];
      ...
      axios__WEBPACK_IMPORTED_MODULE_1___default().get(this.$apiUrl + '/orders/view/' + orderId).then(function (response) {
        ...
        if (order) {
          if (order.order_type === 'selfpickup') {
            _this7.$router.push('/self_pickup_orders/view/' + orderId);
          } else {
            _this7.$router.push('/orders/view/' + orderId);
          }
        } else {
          _this7.$router.push('/orders/view/' + orderId);
        }
        ...
        _c(
          ...
          [
            _c("ul", { staticClass: "navbar-nav ms-auto mb-2 mb-lg-0" }, [
              _vm.showBackButton
                ? _c("li", { staticClass: "nav-item me-2" }, [
                    _c(
                      "button",
                      {
                        staticClass:
                          "btn header-back-btn d-flex align-items-center mt-1",
                        attrs: { type: "button", title: _vm.__("back") },
                        on: { click: _vm.goBack },
                      },
                      [
                        _c("i", {
                        ...
                        _vm._v(" "),
                        _c("span", { staticClass: "ms-1 d-none d-md-inline" }, [
