...
class AdminAuthController extends Controller
{
    ...
    public function logout (Request $request)
    {
        ...
        // Delete FCM token for this device from admin_tokens table
        if (isset($request->fcm_token) && !empty($request->fcm_token)) {
    ...
    public function sellerRegister(Request $request){
        ...
        $validator = Validator::make($requestData,[
            ...
            'fssai_img' => 'required|mimes:jpeg,jpg,png,gif,pdf',
            'store_images' => 'required|array',
            'store_images.*' => 'required|mimes:jpeg,jpg,png,gif,pdf',
        ]);
        ...
        try {
            ...
            $user_data_admin_table = auth()->user();

            $seller = Seller::where('admin_id', $user_data_admin_table->id)->first();

            // dd($user_data_admin_table->id);

            Admin::where('id', $user_data_admin_table->id)->update([
            ...
            $admin = Admin::where('id', $user_data_admin_table->id)->first();
            ...
            $seller->store_location = $request->store_location ?? null;
            $seller->store_city = $request->store_city ?? null;
            $seller->tax_name = $request->tax_name ?? null;
            ...
            $seller->pan_number = $request->pan_number ?? null;
            $seller->city_id = $request->city_id ?? null;
            $seller->store_description = $request->store_description;
            $seller->commission = $request->commission ?? 0;
            ...
            if ($request->hasFile('pan_img')) {
                ...
                $fileName = 'pan_'.time().'_'.rand(1111,99999).'.'.$file->getClientOriginalExtension();
                $path = Storage::disk('public')->putFileAs('sellers', $file, $fileName);
                $seller->pan_img = $path;
            }

            if ($request->hasFile('fssai_img')) {
                $file = $request->file('fssai_img');
                $fileName = 'fssai_'.time().'_'.rand(1111,99999).'.'.$file->getClientOriginalExtension();
                $path = Storage::disk('public')->putFileAs('sellers', $file, $fileName);
                $seller->fssai_img = $path;
            }
            ...
            if ($request->hasFile('store_images')) {
                foreach ($request->file('store_images') as $file) {
                    $fileName = 'store_'.time().'_'.rand(1111,99999).'.'.$file->getClientOriginalExtension();
                    $path = Storage::disk('public')->putFileAs('sellers', $file, $fileName);
                    $storeImagesArray[] = $path;
                }
                $seller->store_images = json_encode($storeImagesArray);
            }
        ...
        try {
            ...
            $data = [
                "seller" => [
                    ...
                    "logo_url" => $seller->logo ? url('storage/'.$seller->logo) : null,
                    "store_url" => $seller->store_url ?? null,
                    "pan_img_url" => $seller->pan_img ? url('storage/'.$seller->pan_img) : null,
                    "fssai_img_url" => $seller->fssai_img ? url('storage/'.$seller->fssai_img) : null,
                    "store_images" => array_map(function($img){
                        return url('storage/'.$img);
                    }, $storeImages),
                    "national_id_card" => $seller->national_identity_card ? url('storage/'.$seller->national_identity_card) : null,
                    "address_proof" => $seller->address_proof ? url('storage/'.$seller->address_proof) : null,
                ]
            ];
