...
class FaqsApiController extends Controller
{
    ...
    public function getByRole(Request $request){
        if (!in_array($request->role, Faq::ROLES)) {
            return CommonHelper::responseError("Invalid role. Allowed values: " . implode(', ', Faq::ROLES));
        }
        $faqs = Faq::where('role', $request->role)->orderBy('id', 'DESC')->get();
        return CommonHelper::responseWithData($faqs);
    }
    ...
    public function save(Request $request){
        ...
        $faq->role = in_array($request->role, Faq::ROLES) ? $request->role : 'customer';
        $faq->save();
    ...
    public function update(Request $request){
        ...
        if(isset($request->id)){
            ...
            $faq->question = $request->question;
            $faq->answer = $request->answer ?? "";
            $faq->role = in_array($request->role, Faq::ROLES) ? $request->role : $faq->role;
            $faq->save();
        }
