...
class WebSettingsApiController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        {
            $filePaths['play_store_logo'] = MediaUploadService::upload(
                $request->file('play_store_logo'),
                'front_end/play_store_logo'
            );
        }

        if($request->hasFile('ios_store_logo'))
        {
            $filePaths['ios_store_logo'] = MediaUploadService::upload(
                $request->file('ios_store_logo'),
                'front_end/ios_store_logo'
            );
        }

        if($request->hasFile('favicon'))
        {
            $filePaths['favicon'] = MediaUploadService::upload(
        ...
        {
            $filePaths['web_logo'] = MediaUploadService::upload(
                $request->file('web_logo'),
                'front_end/web_logo'
            );
        }
        ...
        {
            $filePaths['placeholder_image'] = MediaUploadService::upload(
                $request->file('placeholder_image'),
                'front_end/placeholder_image'
            );
        }
