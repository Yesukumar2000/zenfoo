...
class SettingApiController extends Controller
{
    public function getSettings(Request $request){
        $variables = array(
            ...
            "text_gen_key",
            "bike_moving_animation",
            "customer_login_products_animation",
            "all_stores_img",
            "explore_logo_icon",
            "free_delivery_order_amount",
            "store_placeholder_img_food",
            "store_placeholder_img_veg",
            "store_placeholder_img_grocery",
            "store_placeholder_img_meat",
            "store_placeholder_img_supermart"
        );
        $data = CommonHelper::getSettings($variables);

        // Customer-specific support contacts, falling back to the global pair.
        $data["support_number"] = Setting::support_value('support_number', 'customer');
        $data["support_email"] = Setting::support_value('support_email', 'customer');
    ...
    public function getLogo(){
        ...
        if (!empty($logo)) {
            $logo = str_starts_with($logo, 'http') ? $logo : asset('storage/'.$logo);
    ...
    public function getFrontEndSettings(){
        ...
        if ($front_end_settings) {
            ...
            $data['favicon'] = $faviconVal ? (str_starts_with($faviconVal, 'http') ? $faviconVal : asset('storage/'.$faviconVal)) : '';
    ...
    public function getSeoSettings(Request $request)
    {
        ...
        $data = $query->offset($offset)
                  ...
                  ->map(function ($item) {
                      $faviconVal = Setting::get_value('favicon');
                      $item->favicon = $faviconVal
                          ? (str_starts_with($faviconVal, 'http') ? $faviconVal : asset('storage/' . $faviconVal))
                          : '';
                      return $item;
                  });
    ...
    /**
     * Get app media assets (videos and images)
     * GET /customer/app-media?type=login_animation
     * GET /customer/app-media?type=bike_animation
     * GET /customer/app-media?type=explore_logo
     * GET /customer/app-media (returns all)
     */
    public function getAppMedia(Request $request)
    {
        $type = $request->input('type');

        // Map type parameter to setting variable names
        $typeMap = [
            'login_animation' => 'customer_login_products_animation',
            'bike_animation' => 'bike_moving_animation',
            'all_stores_img' => 'all_stores_img',
            'explore_logo' => 'explore_logo_icon'
        ];

        // If type is specified, return only that asset
        if ($type && isset($typeMap[$type])) {
            $variable = $typeMap[$type];
            $value = \App\Models\Setting::get_value($variable);
            ...
            if (!empty($value)) {
                ...
                return CommonHelper::responseError('Media not found!');
            }
        }

        // If no type specified, return all media assets
        $variables = array(
            "bike_moving_animation",
            "customer_login_products_animation",
            "all_stores_img",
        ...
        if(!empty($data)){
            ...
            return CommonHelper::responseError('No media found!');
        }
    }


    /**
     * Get free delivery order amount setting
     */
    public function getFreeDeliveryAmount(Request $request)
    {
        $value = Setting::get_value('free_delivery_order_amount');
        if ($value !== null) {
            return CommonHelper::responseWithData([
            ...
            return CommonHelper::responseError('Setting not found!');
        }
    }
}
