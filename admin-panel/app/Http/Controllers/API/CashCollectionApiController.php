...
class CashCollectionApiController extends Controller
{
    public function getCashCollection(Request $request){
        ...
        $transactions = DeliveryBoyTransaction::select(
                'orders.*',
                'delivery_boy_transactions.*',
                'delivery_boy_transactions.settled_with_admin',
                'delivery_boy_transactions.settled_at',
                'delivery_boys.name',
                'delivery_boys.mobile',
                'delivery_boys.address'
            )
        ...
        $transactions = $transactions->orderBy('delivery_boy_transactions.id', 'DESC')->get()->toArray();
        ...
        foreach($transactions as $t => $row) {
            if($row['type'] == 'delivery_boy_cash_collection'){
                 ...
                 $transactions[$t]['settlement_status'] = 'Settled';
                 $transactions[$t]['settlement_status_class'] = 'bg-success';
            }elseif($row['type'] == 'COD'){
                $transactions[$t]['collected_amount'] = 0;
                // Check settlement status for COD transactions
                if(isset($row['settled_with_admin']) && $row['settled_with_admin']) {
                    $transactions[$t]['settlement_status'] = 'Settled';
                    $transactions[$t]['settlement_status_class'] = 'bg-success';
                } else {
                    $transactions[$t]['settlement_status'] = 'Pending';
                    $transactions[$t]['settlement_status_class'] = 'bg-warning';
                }
            }
        }
