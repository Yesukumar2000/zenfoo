...
class SellerController extends BaseController
{
    ...
    public function getProducts(Request $request){
        ...
        $products = \DB::select("SELECT p.id AS product_id, p.*, p.name, p.seller_id, p.status, p.tax_id, p.image,
            CASE WHEN p.image LIKE 'http%' THEN p.image ELSE CONCAT('" . asset('storage/') . "/', p.image) END as image_url,
    ...
    public function getWeeklySales(){
        ...
        $year = date("Y");
        $curdate = date('Y-m-d');
        $orders = Order::select(DB::raw('ROUND(SUM(remaining_final), 2) AS total_sale'),
    ...
    public function getOrders(Request $request){
        ...
        if ($filter) {
            $columns = [
                'orders.payment_method', 'orders.id', 'delivery_boys.name','orders.delivery_charge','orders.wallet_balance', 'orders.remaining_final','orders.total','orders.delivery_time','sellers.name',
        ...
        $order_items = Order::select('order_items.*','orders.mobile','orders.total' ,'orders.delivery_charge','orders.discount','orders.promo_code',
            'orders.promo_discount','orders.wallet_balance','orders.final_total','remaining_final','orders.payment_method','orders.address','orders.delivery_time',
    ...
    public function getSelfPickupOrders(Request $request){
        ...
        $orders = Order::select('orders.*','orders.id as order_id','orders.active_status','orders.additional_charges','sellers.name as seller_name',
            'users.name as user_name','order_items.active_status as order_status')
            ->leftJoin('order_items', 'order_items.order_id', '=', 'orders.id')
            ->leftJoin('users', 'orders.user_id', '=', 'users.id')
            ->leftJoin('product_variants', 'order_items.product_variant_id', '=', 'product_variants.id')
        ...
        if ($filter) {
            $columns = [
                'orders.payment_method', 'orders.id','orders.delivery_charge','orders.wallet_balance', 'orders.remaining_final','orders.total','orders.delivery_time','sellers.name',
                'users.name', 'order_items.active_status', 'products.name'
            ];
        ...
        $order_items = Order::select('order_items.*','orders.mobile','orders.total' ,'orders.delivery_charge','orders.discount','orders.promo_code',
            'orders.promo_discount','orders.wallet_balance','orders.final_total','remaining_final','orders.payment_method','orders.delivery_time',
            'users.name as user_name'
            ,'order_items.status as order_status','sellers.name as seller_name')
            ->leftJoin('order_items', 'order_items.order_id', '=', 'orders.id')
            ->leftJoin('users', 'orders.user_id', '=', 'users.id')
    ...
    public function getSettings(){
        ...
        // Seller-specific support contacts, falling back to the global pair.
        $settings["support_number"] = Setting::support_value('support_number', 'seller');
        $settings["support_email"] = Setting::support_value('support_email', 'seller');
