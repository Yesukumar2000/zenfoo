...
/**
 * Read-only diagnostic for the Vendor Commission pipeline. Mirrors
 * vendor-gst:check.
 *
 * Usage:
 *   php artisan vendor-commission:check                 # all sellers
 *   php artisan vendor-commission:check --seller-id=89  # one seller
 *   php artisan vendor-commission:check --drift-only    # only stale rows
 */
class CheckVendorCommissionResolution extends Command
{
    protected $signature = 'vendor-commission:check
                            {--seller-id= : Limit output to a single seller}
                            {--drift-only : Only show rows where snapshot != live}';

    protected $description = 'Read-only diagnostic of the vendor commission resolution pipeline.';
    ...
    public function handle(): int
    {
        ...
        $query = DB::table('sellers as s')
            ...
            ->select(
                's.id',
                's.store_id',
                's.vendor_commission_percent as snapshot',
        ...
        $this->newLine();
        $this->line('To re-snapshot drifted rows: php artisan vendor-commission:backfill --overwrite --commit');
    ...
    {
        $this->info('Current Vendor Commission Configurations (from settings table):');
        $this->table(
            ['variable', 'category', 'value %'],
            [
                ['vendor_commission_vegetables_fruits', 'Vegetables & Fruits', $rates['vendor_commission_vegetables_fruits'] ?? 'unset'],
                ['vendor_commission_chicken_meat',      'Chicken & Meat',      $rates['vendor_commission_chicken_meat']      ?? 'unset'],
                ['vendor_commission_food',              'Food',                $rates['vendor_commission_food']              ?? 'unset'],
                ['vendor_commission_super_mart',        'Super Mart',          $rates['vendor_commission_super_mart']        ?? 'unset'],
            ]
        );
        $this->newLine();
    }
