...
class Paytm
{
    ...
    static public function get_credentials()
    {
        ...
        // Get credentials from new structure
        $paytm_merchant_id = Setting::get_value($prefix . '_merchant_id');
        $paytm_merchant_key = Setting::get_value($prefix . '_merchant_key');
        $paytm_website = Setting::get_value($prefix . '_website') ?? 'WEBSTAGING';
        $paytm_industry_type_id = Setting::get_value($prefix . '_industry_type') ?? 'Retail';
        $paytm_channel_id = Setting::get_value($prefix . '_channel_id') ?? 'WAP';

        // Fallback to old structure if new structure not found
        if (empty($paytm_merchant_id)) {
            $paytm_merchant_id = Setting::get_value('paytm_merchant_id');
            $paytm_merchant_key = Setting::get_value('paytm_merchant_key');
            $paytm_website = Setting::get_value('paytm_website') ?? 'WEBSTAGING';
            $environment = Setting::get_value('paytm_mode') ?? 'test';
        }

        $data['paytm_payment_mode'] = $environment;
        $data['paytm_merchant_key'] = $paytm_merchant_key;
        $data['paytm_merchant_id'] = $paytm_merchant_id;
        $data['paytm_website'] = $paytm_website;
        $data['paytm_industry_type_id'] = $paytm_industry_type_id;
        $data['paytm_channel_id'] = $paytm_channel_id;
        // Updated to official Paytm documentation URLs (paytmpayments.com instead of paytm.in)
        $data['url'] = ($environment === 'live') ? "https://secure.paytmpayments.com/" : "https://securestage.paytmpayments.com/";
    ...
    static public function transaction_status($order_id)
    {
        ...
        $credentials = self::get_credentials();
        // Use the URL from get_credentials() - don't override with old URLs
        // $credentials['url'] is already set correctly in get_credentials()
