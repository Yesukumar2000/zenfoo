...
class CommonHelper
{
    ...
    {
        // Wrap single file in array so foreach works for both cases
        if ($images instanceof \Illuminate\Http\UploadedFile) {
            $images = [$images];
        }

        \Log::info('uploadProductImages called', [
            'product_id' => $product_id,
            'variant_id' => $variant_id,
            'file_count' => is_array($images) ? count($images) : 'not_array',
        ]);

        foreach ($images as $file) {
            $allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
            $extension = $file->getClientOriginalExtension();
            \Log::info('uploadProductImages - processing file', [
                'original_name' => $file->getClientOriginalName(),
                'extension' => $extension,
                'size' => $file->getSize(),
            ]);
            if (in_array(strtolower($extension), $allowedExtensions)) {
                $image = \App\Services\MediaUploadService::uploadWithFullUrl(
                    $file,
                    'products'
                );
                ...
                $productImages->save();
                \Log::info('uploadProductImages - saved to DB', [
                    'product_images_id' => $productImages->id,
                    'image_url' => $image,
                    'variant_id' => $variant_id,
                ]);
            } else {
                \Log::warning('uploadProductImages - skipped file (invalid extension)', [
                    'extension' => $extension,
                    'original_name' => $file->getClientOriginalName(),
                ]);
            }
  ...
  public static function validatePromoCode($user_id, $promo_code, $total)
    {
        ...
        $order_count = Order::where('user_id', $user_id)->where('promo_code_id', $promo_code_id)->count();
        if ($order_count >= 1) {
            $response['is_applicable'] = 0;
            $response['message'] = "already used this";
            return $response;
        }
...
//dd($sellerIds);
    ...
    public static function addWalletTransaction($order_id, $order_item_id, $user_id, $type, $wallet_balance, $mesage, $status = 1, $payment_type = null){
    ...
    // Wallet-transaction message used for referral first-order bonuses. Kept as
    // a constant because both the crediting and the idempotency/stats lookups
    // must match it exactly.
    const REFERRAL_BONUS_MESSAGE = 'Refer Earn First Order Bonus';

    /**
     * Credit a referrer's wallet for the invited user's first qualifying order.
     * Single source of truth for referral crediting — enforces idempotency
     * (never twice for the same order) and the lifetime cap
     * (`max_refer_earn_amount`; 0/empty = unlimited). Order eligibility
     * (first delivered order, min amount, return window) is the caller's job.
     *
     * @return bool true if a credit was applied, false otherwise.
     */
    /**
     * Run the whole referral gate for an order that has just been marked
     * delivered: attribution, minimum amount, first-delivered-order, and the
     * return window (credit now, or defer until the window closes).
     *
     * Every path that marks an order delivered must call this. The driver app
     * sets active_status directly in DeliveryBoy\OrderController rather than
     * going through OrdersApiController@updateStatus, so relying on the admin
     * path alone silently skipped the payout for driver-delivered orders --
     * which is every real order.
     *
     * Safe to call more than once: the credit helper is idempotent per order.
     */
    public static function processReferralForDeliveredOrder($orderId): void
    {
        $order = Order::with('user', 'items.productVariant.product')->find($orderId);
        if (!$order) {
            return;
        }

        $user = $order->user;
        if (!$user || blank($user->friends_code)) {
            return;
        }

        $minOrderAmount = (float) Setting::get_value('referral_min_order_amount');
        if ((float) $order->final_total < $minOrderAmount) {
            return;
        }

        // Only the customer's first delivered order earns the referrer a bonus.
        // Compared by id so a later repeat order can never disqualify the first
        // one, which matters whenever this runs after the fact.
        ...
        // One bonus per referred customer, ever -- this is what keeps the `<`
        // comparison above safe if orders are delivered out of sequence.
        $alreadyPaidForCustomer = WalletTransaction::where('message', self::REFERRAL_BONUS_MESSAGE)
            ->whereIn('order_id', Order::where('user_id', $user->id)->select('id'))
            ->exists();
        if ($alreadyPaidForCustomer) {
            return;
        }
        ...
        // Anything returnable defers the credit until the longest window closes,
        // so a returned order cannot leave a paid-out bonus behind.
        $maxReturnDays = 0;
        ...
        if ($maxReturnDays === 0) {
            self::creditReferralFirstOrderBonus($order, $referrer);
            return;
        }

        $deliveredAt = $order->delivered_at_time
            ? Carbon::parse($order->delivered_at_time)
        ...
        ProcessReferralBonusAfterReturnPeriod::dispatch($order->id)
            ->delay($delay > 0 ? Carbon::now()->addSeconds($delay) : Carbon::now());
    }

    public static function creditReferralFirstOrderBonus($order, $referrer): bool
    {
        if (!$order || !$referrer) {
            return false;
        }

        // Idempotency: never credit the same order's bonus twice.
        $alreadyCredited = WalletTransaction::where('order_id', $order->id)
            ->where('user_id', $referrer->id)
            ->where('message', self::REFERRAL_BONUS_MESSAGE)
            ->exists();
        if ($alreadyCredited) {
            return false;
        }

        $referralCredit = (float) Setting::get_value('referral_credit_first_order');
        if ($referralCredit <= 0) {
            return false;
        }

        // Enforce the lifetime referral cap. 0/empty means no cap.
        $cap = (float) Setting::get_value('max_refer_earn_amount');
        if ($cap > 0) {
            $alreadyEarned = (float) WalletTransaction::where('user_id', $referrer->id)
                ->where('type', 'credit')
                ->where('message', self::REFERRAL_BONUS_MESSAGE)
                ->sum('amount');
            $remaining = $cap - $alreadyEarned;
            if ($remaining <= 0) {
                return false; // cap already reached
            }
            // Credit only up to the remaining headroom.
            $referralCredit = min($referralCredit, $remaining);
        }

        $new_balance = (float) $referrer->balance + $referralCredit;
        self::updateUserWalletBalance($new_balance, $referrer->id);
        self::addWalletTransaction($order->id, 0, $referrer->id, 'credit', $referralCredit, self::REFERRAL_BONUS_MESSAGE);
        return true;
    }
    ...
    public static function isDeliverableOrder($address_id, $latitude, $longitude, $seller_id){
        if (!empty($seller_id) || $seller_id != "") {
            ...
            // If seller doesn't exist or doesn't have coordinates, skip distance calculation
            if (!$seller || empty($seller->latitude) || empty($seller->longitude)) {
                // Just check if city is deliverable
    ...
    public static function getImages($product_id,$variant_id=0,$allImages=false){
        $query = ProductImages::where('product_id',$product_id);
        if(!$allImages){
            $query->where('product_variant_id',$variant_id);
        }
        return $query->get()->pluck('image_url')->toArray();
    }
    ...
    public static function getProductVariant($variant_id, $user_id = null)
{
    ...
    /* customer facing order number, e.g. order 451 => ZF-0451 */
    const ORDER_NUMBER_PREFIX = 'ZF-';
    const ORDER_NUMBER_PAD = 4;

    /**
     * Build the customer facing order number from the order primary key.
     * Padding is a minimum, not a cap - order 12345 becomes ZF-12345.
     */
    public static function formatOrderNumber($order_id){
        if (empty($order_id)) {
            return null;
        }
        return self::ORDER_NUMBER_PREFIX . str_pad((string) $order_id, self::ORDER_NUMBER_PAD, '0', STR_PAD_LEFT);
    }

    /**
     * Reverse of formatOrderNumber, for search boxes and deep links.
     * Accepts 'ZF-0451', 'zf-0451', '#ZF-0451' or plain '451' and returns the id
     * as an integer, or null when the value is not an order number at all.
     */
    public static function parseOrderNumber($value){
        $value = trim((string) $value);
        $value = ltrim($value, '#');
        if (stripos($value, self::ORDER_NUMBER_PREFIX) === 0) {
            $value = substr($value, strlen(self::ORDER_NUMBER_PREFIX));
        }
        $value = ltrim($value, '0');
                ...
                if (isset($result['body']['status']) && $result['body']['status'] == "REQUEST_DENIED") {
                    ...
                    if (isset($result['body']['rows'][0]['elements'][0]['status']) && $result['body']['rows'][0]['elements'][0]['status'] == "OK") {
                        ...
                        if ($charge_method == "per_km_charge") {
                            // Get tiered delivery charges from settings
                            ...
                            // Tiered delivery charge logic
                            if ($distance < 1) {
                                // Less than 1 km
                                $charge = $chargeBelow1km;
                            } elseif ($distance >= 1 && $distance <= 2) {
                                // Between 1 and 2 km
                                $charge = $charge1to2km;
                            } else {
                                // More than 2 km - use per km charge, floored at
                                // the 1-2 km tier so an unconfigured
                                // per_km_charge can never produce a 0 charge.
                                $charge = max(
                                    (float) $city['per_km_charge'] * ceil($distance),
                                    (float) $charge1to2km
                                );
                            }
                        }
                        ...
                        // Free delivery disabled - no free delivery charge will be applied
    ...
    public static function getOrderDetails($order_id, $exclude_cancelled_returned = false){
        $order = Order::select('orders.*','orders.additional_charges','orders.pickup_address','orders.address as order_address','orders.mobile as order_mobile','orders.id as order_id','orders.created_at as orders_created_at','users.*','users.name as user_name','users.email as user_email'
            ...
            'sellers.email as seller_email', 'sellers.store_name', 'sellers.formatted_address as seller_formatted_address', 'sellers.place_name as seller_place_name', 'sellers.latitude as seller_latitude', 'sellers.longitude as seller_longitude', 'sellers.pickup_store_timings', 'delivery_boys.name as delivery_boy_name',
            ...
            // users.* in the select above overwrites orders.id, so the order_number
            // accessor cannot be trusted here - derive it from the orders.id alias.
            $order->orders_id = CommonHelper::formatOrderNumber($order->order_id);
        ...
        $order_items_query = Order::select('order_items.*','orders.mobile','orders.total' ,'orders.delivery_charge','orders.discount','orders.promo_code',
            'orders.promo_discount','orders.wallet_balance','orders.final_total','orders.payment_method','orders.address','orders.delivery_time',
            'users.name as user_name','order_items.status as order_status','sellers.name as seller_name','products.id as product_id','products.return_status','products.return_days',DB::raw('CONCAT("' . asset('storage/') . '", "/", products.image) as image'), 'os.id as active_status', 'os.status as status_name')
            ->leftJoin('order_items', 'order_items.order_id', '=', 'orders.id')
            ->leftJoin('users', 'orders.user_id', '=', 'users.id')
            ->leftJoin('product_variants', 'order_items.product_variant_id', '=', 'product_variants.id')
            ->leftJoin('products', 'product_variants.product_id', '=', 'products.id')
        ...
        $order_items = $order_items_query->orderBy('order_items.id','DESC')->get();
             if(auth()->user()->role_id == 3){
               $seller_id =  auth()->user()->seller->id;
                 ...
                 $seller_order_items_query = Order::select('order_items.*','orders.mobile','orders.total' ,'orders.delivery_charge','orders.discount','orders.promo_code',
            'orders.promo_discount','orders.wallet_balance','orders.final_total','orders.payment_method','orders.address','orders.delivery_time',
            'users.name as user_name','order_items.status as order_status','sellers.name as seller_name','products.id as product_id','products.return_status','products.return_days',DB::raw('CONCAT("' . asset('storage/') . '", "/", products.image) as image'), 'os.id as active_status', 'os.status as status_name')
            ->leftJoin('order_items', 'order_items.order_id', '=', 'orders.id')
            ->leftJoin('users', 'orders.user_id', '=', 'users.id')
        ...
        $combo_items_query = \App\Models\OrderComboItem::where('order_id', $order_id);

        if ($exclude_cancelled_returned) {
            // Exclude cancelled/returned combos if needed
            $combo_items_query->whereNotIn('active_status', ['cancelled', 'returned']);
        }

        // Filter by seller if logged in as seller
        if(auth()->user() && auth()->user()->role_id == 3){
            $seller_id = auth()->user()->seller->id;
            $combo_items_query->where('seller_id', $seller_id);
        }

        $combo_items = $combo_items_query->orderBy('id','DESC')->get();

        return array("order" => $order, "order_items" => $order_items, "combo_items" => $combo_items);
    }
    ...
    public static function getDetailedInvoiceData($id)
    {
        \Log::info("Fetching detailed invoice data for Order ID: " . $id);
        $order = \App\Models\Order::select(
            ...
            'delivery_boys.bonus_type as delivery_boy_bonus_type',
        ...
        $orderItems = \App\Models\OrderItem::select(
            'order_items.*',
            'products.id as product_id',
            'products.name as product_name',
            'products.image as product_image',
            'product_variants.measurement as variant_measurement',
            'product_variants.price as variant_price',
            'units.short_code as unit_code',
        ...
        // Get variant IDs and map to products/stores
        ...
        foreach ($orderItems as $item) {
            ...
            if (!isset($storeWiseItems[$storeId])) {
                $storeWiseItems[$storeId] = [
                    'store_id' => $storeId,
                    'store_name' => $storeNames[$storeId] ?? 'Unknown Store',
                    'items' => []
                ];
            }

            // Format item for invoice
            $storeWiseItems[$storeId]['items'][] = [
                ...
                'variant_measurement' => $item->variant_measurement . ' ' . ($item->unit_code ?? ''),
                ...
                'status_name' => $item->item_status_name
            ];
        }
        ...
        if ($order->delivery_boy_id) {
            $deliveryBoy = \App\Models\DeliveryBoy::find($order->delivery_boy_id);
            if ($deliveryBoy) {
                $deliveryBoyDetails = [
                    'id' => $deliveryBoy->id,
                    'name' => $deliveryBoy->name,
                    'mobile' => $deliveryBoy->mobile,
                    'bonus_type' => $deliveryBoy->bonus_type == 1 ? 'Commission' : 'Fixed',
                    'bonus_percentage' => $deliveryBoy->bonus_percentage,
                    'bonus_min_amount' => $deliveryBoy->bonus_min_amount,
                    'bonus_max_amount' => $deliveryBoy->bonus_max_amount
                ];
            }
        }
        ...
        if ($order->transaction_id) {
            $txn = \App\Models\Transaction::find($order->transaction_id);
            if ($txn) {
                $transaction = [
                    'id' => $txn->id,
                    'txn_id' => $txn->txn_id,
        ...
        return [
            'order' => $order,
            'store_wise_items' => array_values($storeWiseItems),
            'combo_items' => $formattedCombos,
            'delivery_boy_details' => $deliveryBoyDetails,
            'transaction' => $transaction
        ];
    }

    public static function downloadOrderInvoice($order_id){
        \Log::info("Initiating invoice download for Order ID: " . $order_id);
        try {
            $data = CommonHelper::getDetailedInvoiceData($order_id);
            if(!$data || !$data["order"]){
                \Log::warning("Invoice Download Failed: Order #$order_id not found.");
                return CommonHelper::responseError("Order Not found!");
            }

            $invoice = view('invoiceMpdf', $data)->render();
            \Log::info("Invoice view rendered for Order ID: " . $order_id);

            $mpdf = new \Mpdf\Mpdf();
            $mpdf->WriteHTML($invoice);
            
            \Log::info("PDF generated and streaming for Order ID: " . $order_id);
            return $mpdf->Output("Invoice-".CommonHelper::formatOrderNumber($order_id).'.pdf', \Mpdf\Output\Destination::INLINE);
        } catch (\Exception $e) {
            \Log::error("ERROR in downloadOrderInvoice for Order ID: " . $order_id, [
                'error' => $e->getMessage(),
                'line' => $e->getLine(),
                'file' => $e->getFile()
            ]);
            return CommonHelper::responseError("Internal error: " . $e->getMessage());
        }
    }
    ...
    public static function getFirebaseKeys(){
        $firebase_array = array(
            ...
            // Without it the panel cannot obtain an FCM token, so admins get no push.
            "fcm_vapid_key" => "",
            "jsonFile" => ""
        );
    ...
    public static function sendReturnRequestNotification($returnRequest){
        ...
        if ($orderItem && isset($orderItem->seller_id) && $orderItem->seller_id != 0) {
            $sellerTokens = AdminToken::where('user_id', $orderItem->seller_id)
                ->where('type', 'Seller')
                ->get()
                ->pluck('fcm_token', 'platform')
                ->toArray();
        ...
        if (isset($returnRequest->delivery_boy_id) && $returnRequest->delivery_boy_id != 0) {
            $deliveryBoyTokens = AdminToken::where('user_id', $returnRequest->delivery_boy_id)
    ...
    public static function sendLowStockNotification($productVariants){
        try {
            ...
            foreach ($productVariants as $variant) {
                ...
                if ($variant && $variant->stock <= $low_stock_limit) {
    ...
    public static function sendNotification($userTokens,$title,$message,$type='',$type_id=0,$image=''){
            ...
            $data['data']['type'] = $type;
            $data['data']['id'] = $type_id ?? '';
            $data['data']['icon'] = $logo;

            if ($data['data']['type'] == 'category') {
                $category = Category::with('catActiveChilds')->find($type_id);
                if ($category) {
            ...
            if ($data['data']['type'] == 'product') {
                $data['data']['type_slug'] = Product::where('id', $type_id)->value('slug') ?? "";
            }
    ...
    public static function productAverageRating($product_id)
    {
        ...
        // Calculate star ratings from order_product_ratings table
        $data['rating_count'] = $ratingCount;
        $data['average_rating'] = $averageRating;
        $data['one_star_rating'] = DB::table('order_product_ratings')->where('product_id', $product_id)->where('rating', 1)->count() ?? 0;
        $data['two_star_rating'] = DB::table('order_product_ratings')->where('product_id', $product_id)->where('rating', 2)->count() ?? 0;
        $data['three_star_rating'] = DB::table('order_product_ratings')->where('product_id', $product_id)->where('rating', 3)->count() ?? 0;
        $data['four_star_rating'] = DB::table('order_product_ratings')->where('product_id', $product_id)->where('rating', 4)->count() ?? 0;
        $data['five_star_rating'] = DB::table('order_product_ratings')->where('product_id', $product_id)->where('rating', 5)->count() ?? 0;
        return $data;
    }
    ...
    public static function estimateTravelTime($distanceKm, $speedKmh = 30)
    {
        if ($speedKmh <= 0) return "Invalid speed";
        ...
        if ($minutes < 1) {
            ...
            $minutesPart = round($minutes % 60);
            return "{$hoursPart} hr " . ($minutesPart ? "{$minutesPart} minutes" : "");
        }
    }
